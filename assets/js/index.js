...
const WEATHER_URL =
  'https://api.open-meteo.com/v1/forecast?latitude=47.8517&longitude=35.1171&hourly=temperature_2m&daily=temperature_2m_max&current_weather=true&timezone=Europe/Kiev'
const threeDaysWeatherUrl =
  'https://api.open-meteo.com/v1/forecast?latitude=47.8517&longitude=35.1171&daily=temperature_2m_max,precipitation_sum,wind_speed_10m_max&wind_speed_unit=ms&timezone=GMT&forecast_days=3'
const sevenDaysWeatherUrl =
  'https://api.open-meteo.com/v1/forecast?latitude=47.875&longitude=35.125&daily=temperature_2m_max,precipitation_sum,wind_speed_10m_max&wind_speed_unit=ms&timezone=GMT&forecast_days=7'
...
function loadWeather (isKMToH, isCelsiDegree) {
  fetch(
    `${WEATHER_URL}${isCelsiDegree ? '' : '&temperature_unit=fahrenheit'}${
      isKMToH ? '' : '&wind_speed_unit=ms'
    }`
  )
...
const showSevenDaysWeatherBtn = document.querySelector(
  '.show-seven-days-weather-btn'
)
showSevenDaysWeatherBtn.onclick = showSevenDaysWeather

function showSevenDaysWeather () {
  const sevenDaysMainContainer = document.querySelector(
    '.seven-days-main-container'
  )
  if (
    sevenDaysMainContainer.style.display === 'none' ||
    !sevenDaysMainContainer.style.display
  ) {
...
function generateWeather ({
  ...
  let colorFunc = isCelsiDegree
    ? calcTemperatureColor
    : calcTemperatureColorFahrenheit
...
function generateThreeDaysWeather (daily) {
  ...
  const threeDaysWeatherCards = document.querySelectorAll(
    '.threedays-weather-card'
  )
...
function generateSevenDaysWeather (daily) {
  ...
  const sevenDaysWeatherCards = document.querySelectorAll(
    '.seven-days-weather-card'
  )
